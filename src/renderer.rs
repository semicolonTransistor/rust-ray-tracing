...
impl TileRenderTask {
    ...
    fn render_vectorized4(&self, camera: &Arc<Camera>, scene: &Arc<Scene>, max_bounces: usize, samples_per_pixel: usize, thread_id: usize) -> TileRenderResult {
        const N:usize = 4;
        let mut result = vec![Rgb::<u8>([0, 0, 0]); self.block_size.pow(2)];
        ...
        for j in 0..self.size_y {
            for i in 0..self.size_x {
                ...
                let pixel = scene.trace_vectorized4(&mut ray_chunks, max_bounces) / (samples_per_pixel as f64);
